...
map("n", "<leader>sw", [[:%s/\<<C-r><C-w>\>/<C-r><C-w>/gI<Left><Left><Left>]], { desc = "Word under Cursor" })
...
lsp.on_attach(function(_, bn)
	...
	map("n", "gd", "<cmd>TroubleToggle lsp_definitions<cr>", { buffer = bn, desc = "Definitions" })
	map("n", "gr", "<cmd>TroubleToggle lsp_references<cr>", { buffer = bn, desc = "References" })
	...
	map("n", "<C-h>", buf.signature_help, { buffer = bn, desc = "Signature Help" })
end)
